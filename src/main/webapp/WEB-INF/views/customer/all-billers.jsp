<%@page import="com.techm.entity.Biller"%>
...
<c:if test="${sessionScope.customer != null}">
  ...
  <div class="row container-fluid">
    ...
    <div class="col col-9">
      <div class="row m-2">
        <span class="text text-center fs-4 fw-bold">All Billers</span>
        <div class="col">

		<% List<Biller> AllBillers = (List<Biller>) request.getAttribute("billers");
			for(Biller biller : AllBillers) {
		%>
          <div class="card-body row m-1 border-bottom border-dark">
            <div class="col col-2">
              <img src="https://i.postimg.cc/MpVCcgJT/biller-logo.png" width="100" height="100" alt="Biller Logo">
            </div>
            <div class="col">
              <span class="text fw-bold fs-5 row"><%= biller.getBillerName() %></span>
              <span class="text row">Description : <%= biller.getAbout() %></span>
              <span class="text row">Category : <%= biller.getCategory() %></span>
              <span class="text fw-bold row">Email : <%= biller.getEmail() %></span>
            </div>
           
            <div class="col col-3 ">
              <%-- <a href="/customer/register-biller?billerId=<%=biller.getBillerId()%>&customerId=${customer.customerId}" class="btn btn-info m-1 w-100 disabled">Register</a>             --%>
              <a href="/customer/view-biller-products?billerId=<%=biller.getBillerId()%>" class="btn btn-success m-1 w-100 mt-4">View Products</a>            
            </div>
          </div>
	<% } %>
        </div>
